...
interface PaseSalaViewerProps {
  onPatientUpdated?: () => void;
}

export default function PaseSalaViewer({ onPatientUpdated }: PaseSalaViewerProps) {
  ...
  async function loadPatients() {
    setLoading(true);
    setError(null);
    const { data, error: fetchError } = await getPatientsByDestination('pase_sala');
    if (fetchError) {
      setError('Error al cargar pase de sala');
    } else {
      setPatients(data || []);
    }
    setLoading(false);
  ...
  if (patients.length === 0) {
    return (
      ...
      >
        No hay pacientes en pase de sala
      </div>
    );
  }
