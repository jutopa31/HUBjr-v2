...
import { Calendar, Users, BookOpen, FileText, MessageSquare, Activity, Settings, Bell, Home, Plus, Clock, User, Award, Brain, ChevronRight, CheckCircle, ChevronDown, ChevronUp, CalendarDays, UserCheck, Stethoscope, Calculator, Edit3, Save, X, Trash2, CalendarPlus } from 'lucide-react';
...
`);
  ...
  const [editingWeeklyAssignments, setEditingWeeklyAssignments] = useState(false);
  ...
  const [resourcesSpecialtyFilter, setResourcesSpecialtyFilter] = useState('all');
  const [resourcesTypeFilter, setResourcesTypeFilter] = useState('all');
  ...
  // Biblioteca digital de recursos organizados por especialidades neurológicas
  ...
  const [classboardEvents, setClassboardEvents] = useState([
    { 
      id: 1,
  ...
  const saveToLocalStorage = () => {
    ...
    localStorage.setItem('classboardEvents', JSON.stringify(classboardEvents));
  };
  ...
  const handleAuthentication = () => {
    ...
    setTimeout(() => {
      ...
      setEditingWeeklyAssignments(false);
  ...
  const updateAssignment = (day: string, type: 'residency' | 'intern', field: 'name' | 'task', value: string) => {
    setWeeklyAssignments(prev => ({
      ...prev,
      [day]: {
        ...prev[day as keyof typeof prev],
        [type]: {
          ...prev[day as keyof typeof prev][type],
          [field]: value
        }
      }
    }));
  ...
  const updateEvent = (eventId: number, field: string, value: string) => {
    setClassboardEvents(prev => 
      prev.map(event => 
        event.id === eventId 
          ? { ...event, [field]: value }
          : event
  ...
  const addNewEvent = (eventData: any) => {
    const newEvent = {
      id: Math.max(...classboardEvents.map(e => e.id), 0) + 1,
      title: eventData.title,
      date: eventData.date,
      time: eventData.time,
      type: eventData.type as 'clinical' | 'theory' | 'workshop' | 'research' | 'administrative',
      description: eventData.description || '',
      presenter: eventData.presenter || '',
      location: eventData.location || 'Hospital Nacional Posadas',
      duration: eventData.duration || '60 minutos'
    };
    setClassboardEvents(prev => [...prev, newEvent]);
  ...
  const deleteEvent = (eventId: number) => {
    if (window.confirm('¿Está seguro de que desea eliminar este evento?')) {
      setClassboardEvents(prev => prev.filter(event => event.id !== eventId));
      saveToLocalStorage();
    }
  };
  ...
  const filteredResources = neurologyResources.filter(resource => {
    const matchesSpecialtyFilter = resourcesSpecialtyFilter === 'all' || resource.specialty === resourcesSpecialtyFilter;
    const matchesTypeFilter = resourcesTypeFilter === 'all' || resource.type === resourcesTypeFilter;
    const matchesSearch = resourcesSearch === '' || 
      resource.title.toLowerCase().includes(resourcesSearch.toLowerCase()) ||
      resource.description.toLowerCase().includes(resourcesSearch.toLowerCase()) ||
      resource.keywords.some(keyword => keyword.toLowerCase().includes(resourcesSearch.toLowerCase()));
    return matchesSpecialtyFilter && matchesTypeFilter && matchesSearch;
  });

  // Get unique specialties and types for filter options
  const uniqueSpecialties = [...new Set(neurologyResources.map(resource => resource.specialty))].sort();
  const uniqueTypes = [...new Set(neurologyResources.map(resource => resource.type))].sort();
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-white rounded-lg shadow p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {/* Botones de edición para eventos */}
                  {isAdminMode && (
                                ...
                                {editingEvents ? (
                                  <div className="space-y-2">
                                    <input
                                      ...
                                      className="w-full font-semibold text-gray-900 bg-yellow-50 border border-yellow-200 rounded px-2 py-1 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                    />
                                    <div className="flex space-x-2">
                                      <input
                                        type="date"
                                        ...
                                        className="text-sm text-gray-600 bg-yellow-50 border border-yellow-200 rounded px-2 py-1 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                      />
                                      <input
                                        type="time"
                                        value={event.time}
                              ...
                              <div>
                                ...
                                {editingEvents ? (
                                  <textarea
                                    ...
                                    className="w-full text-gray-600 mb-3 bg-yellow-50 border border-yellow-200 rounded px-2 py-1 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 resize-none"
                                    rows={3}
                                  />
                                ) : (
                                ...
                                {editingEvents ? (
                                  <input
                                    ...
                                    onChange={(e) => updateEvent(event.id, 'presenter', e.target.value)}
                                    className="w-full text-gray-600 bg-yellow-50 border border-yellow-200 rounded px-2 py-1 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                  />
                                ) : (
                              ...
                              <div>
                                ...
                                {editingEvents ? (
                                  <input
                                    ...
                                    className="w-full text-gray-600 mb-3 bg-yellow-50 border border-yellow-200 rounded px-2 py-1 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                  />
                                ) : (
                  ...
                  <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-6">
                    ...
                    <form onSubmit={(e) => {
                      ...
                      const eventData = {
                        ...
                        presenter: formData.get('presenter') as string,
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm font-medium text-blue-900 mb-1">Título del Evento</label>
                          <input
                            name="title"
                            type="text"
                            required
                            className="w-full px-3 py-2 border border-blue-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            placeholder="Ej: Ateneo Clínico - Epilepsia"
                          />
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-blue-900 mb-1">Tipo de Evento</label>
                          <select
                            name="type"
                            required
                            className="w-full px-3 py-2 border border-blue-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        ...
                        <div>
                          <label className="block text-sm font-medium text-blue-900 mb-1">Fecha</label>
                          <input
                            name="date"
                            type="date"
                            required
                        ...
                        <div>
                          <label className="block text-sm font-medium text-blue-900 mb-1">Hora</label>
                          <input
                            name="time"
                            type="time"
                            required
                        ...
                        <div>
                          <label className="block text-sm font-medium text-blue-900 mb-1">Presentador</label>
                          <input
                            name="presenter"
                            type="text"
                            className="w-full px-3 py-2 border border-blue-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            placeholder="Ej: Dr. García - R3"
                          />
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-blue-900 mb-1">Duración</label>
                          <input
                            name="duration"
                            type="text"
                            className="w-full px-3 py-2 border border-blue-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-blue-900 mb-1">Ubicación</label>
                        <input
                          name="location"
                          type="text"
                          className="w-full px-3 py-2 border border-blue-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                          placeholder="Ej: Aula Magna, 2do piso"
                          defaultValue="Hospital Nacional Posadas"
                        />
                      </div>
                      <div>
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <>
                      {editingWeeklyAssignments ? (
                        <div className="flex space-x-2">
                          ...
                          <button
                            onClick={() => setEditingWeeklyAssignments(false)}
                            className="flex items-center space-x-2 px-3 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 text-sm"
                          >
                            <X className="h-4 w-4" />
                            <span>Cancelar</span>
                        ...
                        <button
                          onClick={() => setEditingWeeklyAssignments(true)}
                  ...
                  {!isAdminMode && (
                    <button
                      ...
                      className="flex items-center space-x-2 px-3 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 text-sm"
                    >
                        ...
                        <div className="bg-white rounded-lg p-3 border-l-4 border-blue-500">
                          ...
                          {editingWeeklyAssignments ? (
                            <div className="space-y-2">
                              <input
                                type="text"
                                value={assignments.residency.name}
                                onChange={(e) => updateAssignment(day, 'residency', 'name', e.target.value)}
                                className="w-full text-sm font-medium text-gray-900 bg-yellow-50 border border-yellow-200 rounded px-2 py-1 focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
                                placeholder="Nombre del residente"
                              />
                              <textarea
                                value={assignments.residency.task}
                                onChange={(e) => updateAssignment(day, 'residency', 'task', e.target.value)}
                                className="w-full text-xs text-gray-600 bg-yellow-50 border border-yellow-200 rounded px-2 py-1 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 resize-none"
                                rows={2}
                        ...
                        <div className="bg-white rounded-lg p-3 border-l-4 border-green-500">
                          ...
                          {editingWeeklyAssignments ? (
                            <div className="space-y-2">
                              <input
                                type="text"
                                value={assignments.intern.name}
                                onChange={(e) => updateAssignment(day, 'intern', 'name', e.target.value)}
                                className="w-full text-sm font-medium text-gray-900 bg-yellow-50 border border-yellow-200 rounded px-2 py-1 focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
                                placeholder="Nombre del interno"
                              />
                              <textarea
                                value={assignments.intern.task}
                                onChange={(e) => updateAssignment(day, 'intern', 'task', e.target.value)}
                                className="w-full text-xs text-gray-600 bg-yellow-50 border border-yellow-200 rounded px-2 py-1 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 resize-none"
                                rows={2}
                                placeholder="Tarea asignada"
                              />
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-green-600 to-blue-700 text-white p-6 rounded-lg">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-green-100 text-lg">Gestión de calendario y eventos académicos</p>
                </div>
              </div>
            </div>
            ...
            <GoogleCalendarIntegration
              events={classboardEvents.map(event => ({
                id: event.id.toString(),
                title: event.title,
                date: event.date,
                time: event.time,
                type: event.type as 'clinical' | 'theory' | 'workshop' | 'research' | 'administrative',
              ...
              onEventSync={(event) => {
                console.log('Event synced to Google Calendar:', event);
              }}
              onImportEvents={(events) => {
                setImportedEvents(events);
                console.log('Events imported from Google Calendar:', events);
              }}
            />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  {classboardEvents.map((event) => (
                    <div key={event.id} className="border rounded-lg p-3 hover:shadow-md transition-shadow">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <div className="flex items-center space-x-2">
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                            event.type === 'clinical' ? 'bg-red-100 text-red-800' :
                            event.type === 'theory' ? 'bg-blue-100 text-blue-800' :
                            event.type === 'workshop' ? 'bg-green-100 text-green-800' :
                            event.type === 'research' ? 'bg-purple-100 text-purple-800' :
                            'bg-gray-100 text-gray-800'
                          }`}>
                            {event.type === 'clinical' ? 'Clínico' :
                             event.type === 'theory' ? 'Teórico' :
                             event.type === 'workshop' ? 'Taller' :
                             event.type === 'research' ? 'Investigación' :
                             'Administrativo'}
                          </span>
                          {isAdminMode && editingEvents && (
                            <button
                      ...
                      <div className="text-sm text-gray-600">
                        ...
                        <p>{event.location}</p>
                        {event.presenter && <p>Presentador: {event.presenter}</p>}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-white rounded-lg shadow p-6">
                <h3 className="text-lg font-semibold mb-4 flex items-center">
                  ...
                  Eventos Importados de Google Calendar
                </h3>
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {importedEvents.length > 0 ? (
                    ...
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-sm">Conecta tu Google Calendar para importar eventos</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                >
                  ...
                  <div className="text-left">
                    <p className="font-medium text-blue-900">Actividades Académicas</p>
                    <p className="text-sm text-blue-600">Ver todas las actividades</p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="text-left">
                    <p className="font-medium text-green-900">Panel Principal</p>
                    <p className="text-sm text-green-600">Ver resumen general</p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="text-left">
                    <p className="font-medium text-purple-900">Registro Asistencial</p>
                    <p className="text-sm text-purple-600">Registrar actividades</p>
                  </div>
                </button>
              </div>
            </div>
            
            {/* Formulario para nuevo evento en sección Schedule */}
            {showNewEventForm && (
            ...
            <div className="bg-blue-50 rounded-lg p-6">
              ...
              <ul className="text-sm text-blue-700 space-y-1">
                <li>• Sincroniza eventos de HUBJR con tu Google Calendar personal</li>
                <li>• Importa eventos externos desde Google Calendar</li>
                <li>• Recibe notificaciones automáticas de actividades próximas</li>
                <li>• Comparte calendarios con compañeros de residencia</li>
                <li>• Accede a tu cronograma desde cualquier dispositivo</li>
              </ul>
            </div>
          </div>
        );
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-indigo-100 text-lg">Biblioteca Digital de Neurología</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-indigo-100 text-sm">{filteredResources.length} recursos disponibles</p>
                  <p className="text-white font-semibold">{uniqueSpecialties.length} especialidades</p>
                </div>
              </div>
            </div>
              ...
              <div className="lg:col-span-1">
                <div className="bg-white rounded-lg shadow p-6 sticky top-6">
                  ...
                  <div className="mb-4">
                    ...
                    <select
                      value={resourcesSpecialtyFilter}
                      onChange={(e) => setResourcesSpecialtyFilter(e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                    >
                  ...
                  <button
                    onClick={() => {
                      setResourcesSearch('');
                      setResourcesSpecialtyFilter('all');
                      setResourcesTypeFilter('all');
                    }}
